...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ============================================================================
    // These enhancements ensure the generated PowerPoint has meaningful content
    // and proper formatting that works well within PowerPoint's display constraints

    // RESEARCH DATA FALLBACK: Use research insights when HTML parsing yields no bullet points
    // This ensures slides always have substantive content even when HTML extraction fails
    // Priority: HTML-extracted content > Research-derived content > Empty slide
    if (slideContent.bulletPoints.length === 0 && researchData) {
      slideContent.bulletPoints = extractKeyPointsFromResearch(researchData);
    }

    // TITLE LENGTH OPTIMIZATION: Prevent title overflow in PowerPoint slide layouts
    // PowerPoint has limited title space - truncate long titles to maintain visual balance
    // 60 characters is the optimal length for most PowerPoint title placeholders
    if (slideContent.title && slideContent.title.length > 60) {
      slideContent.title = slideContent.title.substring(0, 57) + '...';
    }
...
function extractContentFromHtml(html?: string) {
  ...
  try {
    // HTML PARSING: Use multiple patterns to extract different content types
    // Enhanced patterns to catch various HTML structures from OpenAI
    ...
    const subtitleMatch = html.match(/<h2[^>]*>(.*?)<\/h2>/i) ||
                         html.match(/<h3[^>]*>(.*?)<\/h3>/i) ||
                         html.match(/<div[^>]*class="[^"]*subtitle[^"]*"[^>]*>(.*?)<\/div>/i);
    
    const listMatch = html.match(/<ul[^>]*>(.*?)<\/ul>/is) ||
                     html.match(/<ol[^>]*>(.*?)<\/ol>/is);
    
    // Enhanced statistics patterns
    const statMatches = html.match(/<span class="stat-number"[^>]*>(.*?)<\/span>/g) ||
                       html.match(/<div[^>]*class="[^"]*stat[^"]*"[^>]*>(.*?)<\/div>/g) ||
                       html.match(/\b(\d+(?:\.\d+)?%?)\b/g); // Fallback: any numbers

    // CONTENT EXTRACTION: Clean HTML tags and extract plain text content
    const title = titleMatch ? cleanHtmlText(titleMatch[1]) : null;
    const subtitle = subtitleMatch ? cleanHtmlText(subtitleMatch[1]) : null;

    // BULLET POINTS EXTRACTION: Parse lists into array of strings
    let bulletPoints: string[] = [];
    if (listMatch) {
      const liMatches = listMatch[1].match(/<li[^>]*>(.*?)<\/li>/g);
      if (liMatches) {
        bulletPoints = liMatches
          .map(li => cleanHtmlText(li.replace(/<\/?li[^>]*>/g, '')))
          .filter(point => point.length > 0 && point.length < 200); // Filter reasonable lengths
      }
    }

    // FALLBACK: Extract paragraphs as bullet points if no lists found
    if (bulletPoints.length === 0) {
      const paragraphs = html.match(/<p[^>]*>(.*?)<\/p>/g);
      if (paragraphs) {
        bulletPoints = paragraphs
          .map(p => cleanHtmlText(p.replace(/<\/?p[^>]*>/g, '')))
          .filter(point => point.length > 20 && point.length < 150)
    ...
    if (statMatches) {
      statMatches.forEach((match, index) => {
        ...
        if (value && /\d/.test(value)) { // Must contain at least one digit
          statistics.push({ 
            value, 
            label: `Metric ${index + 1}` // Could be enhanced with context
          });
        }
      });
    }
    ...
    return {
      title,
      subtitle,
      bulletPoints: bulletPoints.slice(0, 6), // Limit bullet points for slide readability
      statistics: statistics.slice(0, 3) // Limit statistics for visual balance
    };
  } catch (error) {
