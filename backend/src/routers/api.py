...
async def find_similar_nodes_edges(request: SimilarityRequest):
    ...
    try:
        logger.info(
            f"Finding similar elements for client: {request.client_id}")
        ...
        if request.node_id:
            similar_nodes = kg_service.get_similar_nodes(
                request.node_id, request.top_k)
        ...
        if request.source and request.target:
            similar_edges = kg_service.get_similar_edges(
                request.source, request.target, request.top_k)
